...
assignment that minimizes the total cost. In each game round, $N$ is chosen randomly
...
Because earlier decisions are never reconsidered, the gredy algorithm does not
...
\begin{itemize}
    ...
    \item \textbf{Prefer Hungarian} when the exact minimum cost is required and $n$
          ...
          timetabling, or any domain whre a suboptimal assignment incurs a
...
\begin{itemize}
    \item Greedy times are small enough that the amber bars are barely visible on the
